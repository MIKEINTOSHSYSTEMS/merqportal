...
// Local FormApp configuration
define('BASE_URL', '/apps/formapps/api/v1');
...
// Alternative method using file_get_contents for local requests
function fetchFromAPILocal($endpoint, $params = [])
{
    $baseUrl = getBaseUrl();
    $url = $baseUrl . BASE_URL . $endpoint;
    ...
    try {
        ...
        error_log("Local API request failed: " . $e->getMessage());
    }

    return false;
}

// Get all submissions (with fallback method)
function getSubmissions()
{
    ...
    if (!$submissions) {
        $submissions = fetchFromAPILocal('/forms/' . FORM_ID . '/submissions', [
            'expand' => 'files,comments'
        ]);
    }
